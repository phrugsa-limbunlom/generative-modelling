...
class Config:
    ...
    hidden: int = 256
    n_layers: int = 4
    save_dir: str = "outputs/diffusion/trained"
    save_dir_untrained: str = "outputs/diffusion/untrained"
...
def save_grid(frames, t_vals, cfg, tag=""):
    ...
    if tag == "untrained":
        title = "DDPM Model - Sample Trajectories (untrained)"
    else:
        title = "DDPM Model - Sample Trajectories (trained)"
    ...
    png_path = os.path.join(cfg.save_dir, f"diffusion_grid{suffix}.png")
...
def save_samples(samples, cfg, tag=""):
    suffix = f"_{tag}" if tag else ""
    npy_path = os.path.join(cfg.save_dir, f"diffusion_samples{suffix}.npy")
    ...
    try:
        ...
        plt.axis("equal")
        plt.axis("off")
        png_path = os.path.join(cfg.save_dir, f"diffusion_samples{suffix}.png")
...
if __name__ == "__main__":
    ...
    if cfg.sample_before_train:
        untrained = NoiseNet(dim=2, hidden=cfg.hidden, n_layers=cfg.n_layers).to(cfg.device)
